...
public:
	...
	template <typename U> auto& operator =(const U& b) { x = b; return *this; }
	auto& operator=(const modular& rhs) { x = rhs.x; return *this; }
	...
	auto operator++(int) { int r = x; ++x; return r; }
	auto operator--(int) { int r = x; --x; return r; }
	auto operator-(int) { return !x ? 0 : mod - x; }
	auto operator==(const modular& rhs) { return x == rhs.x; }
	auto operator!=(const modular& rhs) { return x != rhs.x; }

	friend string to_string(modular& m) { return to_string(m.x); }
	friend auto operator+(const modular& lhs, const modular& rhs) { return modular(lhs) += rhs; }
	friend auto operator-(const modular& lhs, const modular& rhs) { return modular(lhs) -= rhs; }
	friend auto operator*(const modular& lhs, const modular& rhs) { return modular(lhs) *= rhs; }
	friend auto operator/(const modular& lhs, const modular& rhs) { return modular(lhs) /= rhs; }
	friend auto operator==(const modular& lhs, const modular& rhs) { return lhs.x == rhs.x; }
	friend auto operator!=(const modular& lhs, const modular& rhs) { return lhs.x != rhs.x; }
